const KEY="budgetTrackerV2";
let data=JSON.parse(localStorage.getItem(KEY)||localStorage.getItem("budgetTrackerV1")||'{"paychecks":[],"bills":[],"spending":[],"debt":[],"goals":[]}');
const $=id=>document.getElementById(id);
const money=n=>new Intl.NumberFormat("en-US",{style:"currency",currency:"USD"}).format(Number(n)||0);
const today=()=>new Date().toISOString().slice(0,10);
function save(){localStorage.setItem(KEY,JSON.stringify(data));render()}
function esc(s){return String(s??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]))}
function currentIncome(){return data.paychecks.filter(x=>x.date<=today()).reduce((a,x)=>a+Number(x.amount),0)}
function upcomingPaycheck(){return data.paychecks.filter(x=>x.date>today()).sort((a,b)=>a.date.localeCompare(b.date))[0]}
function render(){
 const income=currentIncome();
 const spent=data.spending.filter(x=>x.date<=today()).reduce((a,x)=>a+Number(x.amount),0);
 const bills=data.bills.filter(x=>!x.paid).reduce((a,x)=>a+Number(x.amount),0);
 const debt=data.debt.reduce((a,x)=>a+Number(x.balance),0);
 const next=upcomingPaycheck();
 const safe=income-spent-bills;
 $("balance").textContent=money(income-spent);
 ...
 $("safeNote").textContent=next
   ? `Current money only. Next paycheck: ${money(next.amount)} on ${next.date}.`
   : "Based on current income, unpaid bills, and spending.";
 list("paycheckList",data.paychecks,(x,i)=>{
   const future=x.date>today();
   return `<div class="item"><div><b>${money(x.amount)} ${future?"• Upcoming":""}</b><small>${esc(x.note||"Paycheck")} • ${esc(x.date)}</small></div><div class="right"><button class="danger" onclick="del('paychecks',${i})">Delete</button></div></div>`
 });
 ...
 const recent=data.spending.slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,5);
 $("recent").innerHTML=recent.length?recent.map(x=>`<div class="item"><div><b>${esc(x.name)}</b><small>${esc(x.date)}</small></div><b>${money(x.amount)}</b></div>`).join(""):`<div class="empty">No spending yet.</div>`;
}
